...
export function Header({title='班主任评语助手',onBack}:{title?:string;onBack?:()=>void}) {
  ...
  return <header className="flex items-center justify-between px-6 py-3 bg-white border-b border-gray-200 shrink-0">
    ...
    <div className="flex items-center gap-2 text-xs max-w-[68vw] overflow-x-auto shrink"><Badge color={activation.active?'green':'red'}>{activation.active?'当前激活':'未激活'}</Badge><Badge><Calendar size={13}/>有效期至：{activation.expiresAt.slice(0,10)}</Badge><Badge color="blue"><Zap size={13}/>AI剩余：{activation.remainingAiCount}次</Badge><Badge><Users size={13}/>学生：{students.length}</Badge><Badge color="green"><CheckCircle2 size={13}/>已生成：{generated}</Badge></div>
  </header>;
}
